...
public class Settings implements net.ess3.api.ISettings
{
	...
	private List<Integer> _getItemSpawnBlacklist()
	{
		...
		for (String itemName : config.getString("item-spawn-blacklist", "").split(","))
		{
			...
			{
				logger.log(Level.SEVERE, tl_("unknownItemInList", itemName, "item-spawn-blacklist"));
			}
		}
		return epItemSpwn;
	}
	...
	private List<EssentialsSign> _getEnabledSigns()
	{
		...
		for (String signName : config.getStringList("enabledSigns"))
		{
			...
			{
				logger.log(Level.SEVERE, tl_("unknownItemInList", signName, "enabledSigns"));
				continue;
			}
			signsEnabled = true;
		}
	...
	public List<Integer> getProtectList(final String configName)
	{
		...
		for (String itemName : config.getString(configName, "").split(","))
		{
			...
			{
				logger.log(Level.SEVERE, tl_("unknownItemInList", itemName, configName));
			}
		}
		return list;
	}
