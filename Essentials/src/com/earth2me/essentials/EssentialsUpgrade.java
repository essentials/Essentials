...
public class EssentialsUpgrade
{
	...
	private void moveWorthValuesToWorthYml()
	{
		...
		{
			LOGGER.log(Level.SEVERE, tl_("upgradingFilesError"), e);
		}
	}
	...
	private void removeLinesFromConfig(File file, String regex, String info) throws Exception
	{
		...
		if (needUpdate)
		{
			...
			{
				throw new Exception(tl_("configFileMoveError"));
			}
			if (!tempFile.renameTo(file))
			{
				throw new Exception(tl_("configFileRenameError"));
			}
		}
		else
		{
			...
			for (int i = 0; i < listOfFiles.length; i++)
			{
				...
				if (listOfFiles[i].isFile() && filename.endsWith(".dat"))
				{
					try
					{
						...
						{
							throw new Exception(tl_("fileRenameError", filename));
						}
					}
					catch (Exception ex)
					{
			...
			try
			{
				...
				try
				{
					for (String[] parts = new String[0]; rx.ready(); parts = rx.readLine().split(":"))
					{
						...
						{
							throw new Exception(tl_("fileRenameError", "warps.txt"));
						}
					}
				}
				finally
	...
	private void sanitizeAllUserFilenames()
	{
		...
		for (File listOfFile : listOfFiles)
		{
			...
			{
				LOGGER.log(Level.WARNING, tl_("userdataMoveError", filename, sanitizedFilename));
				continue;
			}
			if (newFile.exists())
			{
				LOGGER.log(Level.WARNING, tl_("duplicatedUserdata", filename, sanitizedFilename));
				continue;
			}
			if (!tmpFile.renameTo(newFile))
			{
				LOGGER.log(Level.WARNING, tl_("userdataMoveBackError", sanitizedFilename, sanitizedFilename));
			}
		}
		...
		if (configFile.exists())
		{
			...
			try
			{
				...
				if (!config.hasProperty("spawns"))
				{
					...
					{
						throw new Exception(tl_("fileRenameError", "spawn.yml"));
					}
		...
		if (configFile.exists())
		{
			...
			try
			{
				...
				if (!config.hasProperty("jails"))
				{
					...
					{
						throw new Exception(tl_("fileRenameError", "jail.yml"));
					}
					PrintWriter writer = new PrintWriter(configFile);
					try
					{
