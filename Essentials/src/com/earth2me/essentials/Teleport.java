...
public class Teleport implements net.ess3.api.ITeleport
{
	...
	public void cooldown(boolean check) throws Exception
	{
		...
		if (teleportOwner.getLastTeleportTimestamp() > 0)
		{
			...
			else if (lastTime > earliestLong && !teleportOwner.isAuthorized("essentials.teleport.cooldown.bypass"))
			{
				...
				throw new Exception(tl_("timeBeforeTeleport", DateUtil.formatDateDiff(time.getTimeInMillis())));
			}
		}
	...
	private void warnUser(final IUser user, final double delay)
	{
		...
		user.sendMessage(tl_("dontMoveMessage", DateUtil.formatDateDiff(c.getTimeInMillis())));
	}
	...
	public void now(Player entity, boolean cooldown, TeleportCause cause) throws Exception
	{
		...
		now(teleportOwner, target, cause);
		teleportOwner.sendMessage(tl_("teleporting", target.getLocation().getWorld().getName(), target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
	}
	...
	protected void now(IUser teleportee, ITarget target, TeleportCause cause) throws Exception
	{
		...
		if (LocationUtil.isBlockUnsafe(loc.getWorld(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()))
		{
			...
			{
				throw new Exception(tl_("unsafeTeleportDestination", loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
			}
		}
		else
		{
	...
	public void teleportPlayer(IUser teleportee, Player entity, Trade chargeFor, TeleportCause cause) throws Exception
	{
		...
		teleportee.sendMessage(tl_("teleporting", target.getLocation().getWorld().getName(), target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
		teleportOwner.sendMessage(tl_("teleporting", target.getLocation().getWorld().getName(), target.getLocation().getBlockX(), target.getLocation().getBlockY(), target.getLocation().getBlockZ()));
	}
	...
	public void warp(IUser teleportee, String warp, Trade chargeFor, TeleportCause cause) throws Exception
	{
		...
		teleportee.sendMessage(tl_("warpingTo", warp, loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
		teleport(teleportee, new LocationTarget(loc), chargeFor, cause);
	}
	...
	public void back(Trade chargeFor) throws Exception
	{
		...
		teleportOwner.sendMessage(tl_("backUsageMsg", loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
