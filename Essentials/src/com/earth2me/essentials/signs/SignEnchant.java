...
public class SignEnchant extends EssentialsSign
{
	...
	protected boolean onSignCreate(final ISign sign, final User player, final String username, final IEssentials ess) throws SignException, ChargeException
	{
		...
		if (enchantLevel.length != 2)
		{
			...
			throw new SignException(tl_("invalidSignLine", 3));
		}
		final Enchantment enchantment = Enchantments.getByName(enchantLevel[0]);
		if (enchantment == null)
		{
			sign.setLine(2, "§c<enchant>");
			throw new SignException(tl_("enchantmentNotFound"));
		}
		int level;
		try
		{
	...
	protected boolean onSignInteract(ISign sign, User player, String username, IEssentials ess) throws SignException, ChargeException
	{
		...
		{
			throw new SignException(tl_("missingItems", 1, sign.getLine(1)));
		}
		if (search != null && playerHand.getType() != search.getType())
		{
			throw new SignException(tl_("missingItems", 1, search.getType().toString().toLowerCase(Locale.ENGLISH).replace('_', ' ')));
		}
		...
		if (level == 0)
		{
			player.sendMessage(tl_("enchantmentRemoved", enchantmentName.replace('_', ' ')));
		}
		else
		{
			player.sendMessage(tl_("enchantmentApplied", enchantmentName.replace('_', ' ')));
		}
