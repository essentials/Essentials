...
public class Commanditemdb extends EssentialsCommand
{
	...
	protected void run(Server server, CommandSource sender, String commandLabel, String[] args) throws Exception
	{
		...
		sender.sendMessage(tl_("itemType", itemStack.getType().toString(), itemStack.getTypeId() + ":" + Integer.toString(itemStack.getDurability())));
		...
		if (itemHeld && itemStack.getType() != Material.AIR)
		{
			...
			if (maxuses != 0)
			{
				sender.sendMessage(tl_("durability", Integer.toString(durability)));
			}
		}
		final String itemNameList = ess.getItemDb().names(itemStack);
		if (itemNameList != null)
		{
			sender.sendMessage(tl_("itemNames", ess.getItemDb().names(itemStack)));
		}
	}
}
