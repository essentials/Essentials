...
{
	final String translatedMe = tl_("me");
	...
	public void run(Server server, CommandSource sender, String commandLabel, String[] args) throws Exception
	{
		...
		if (args[0].equalsIgnoreCase(Console.NAME))
		{
			...
			sender.sendMessage(tl_("msgFormat", translatedMe, Console.NAME, message));
			CommandSender cs = Console.getCommandSender(server);
			cs.sendMessage(tl_("msgFormat", senderName, translatedMe, message));
	...
	{		
		...
		{
			sender.sendMessage(tl_("userAFK", matchedUser.getDisplayName()));
		}

		sender.sendMessage(tl_("msgFormat", translatedMe, matchedUser.getDisplayName(), args[0]));
		if (sender.isPlayer() && matchedUser.isIgnoredPlayer(ess.getUser(sender.getPlayer())))
		{
			return;
		}

		matchedUser.sendMessage(tl_("msgFormat", senderName, translatedMe, args[0]));
