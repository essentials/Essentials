...
public class Commandtempban extends EssentialsCommand
{
	...
	public void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		...
		if (!user.isOnline())
		{
			...
			{
				sender.sendMessage(tl_("tempbanExemptOffline"));
				return;
			}
		}
		else
		{
			...
			{
				sender.sendMessage(tl_("tempbanExempt"));
				return;
			}
		}
		...
		{
			sender.sendMessage(tl_("oversizedTempban"));
			throw new NoChargeException();
		}

		final String senderName = sender.isPlayer() ? sender.getPlayer().getDisplayName() : Console.NAME;
		final String banReason = tl_("tempBanned", DateUtil.formatDateDiff(banTimestamp), senderName);
		user.setBanReason(banReason);
		user.setBanTimeout(banTimestamp);
		user.setBanned(true);
		user.kickPlayer(banReason);

		ess.broadcastMessage("essentials.ban.notify", tl_("playerBanned", senderName, user.getName(), banReason, DateUtil.formatDateDiff(banTimestamp)));
	}
}
