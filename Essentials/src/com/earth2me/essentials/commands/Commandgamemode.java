...
public class Commandgamemode extends EssentialsCommand
{
	...
	protected void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		user.sendMessage(tl_("gameMode", tl_(user.getGameMode().toString().toLowerCase(Locale.ENGLISH)), user.getDisplayName()));
	}
	...
	private void gamemodeOtherPlayers(final Server server, final CommandSource sender, final GameMode gameMode, final String name) throws NotEnoughArgumentsException, PlayerNotFoundException
	{
		...
		{
			throw new NotEnoughArgumentsException(tl_("gameModeInvalid"));
		}
		...
		for (Player matchPlayer : matchedPlayers)
		{
			...
			sender.sendMessage(tl_("gameMode", tl_(player.getGameMode().toString().toLowerCase(Locale.ENGLISH)), player.getDisplayName()));
		}
		if (!foundUser)
		{
