...
public class Commandr extends EssentialsCommand
{
	...
	public void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		...
		{
			throw new Exception(tl_("foreverAlone"));
		}

		final String targetName = target.isPlayer() ? target.getPlayer().getDisplayName() : Console.NAME;

		sender.sendMessage(tl_("msgFormat", tl_("me"), targetName, message));
		if (target.isPlayer())
		{
		...
		target.sendMessage(tl_("msgFormat", senderName, tl_("me"), message));
