...
public class Commandsudo extends EssentialsCommand
{
	...
	public void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		...
		if(args[1].toLowerCase(Locale.ENGLISH).startsWith("c:"))
		{
			...
			{
				throw new Exception(tl_("sudoExempt"));
			}
		...
		sender.sendMessage(tl_("sudoRun", user.getDisplayName(), command, getFinalArg(arguments, 0)));
		...
		if (execCommand != null)
		{
			...
			sender.sendMessage(tl_("errorCallingCommand", command));
		}
	}
}
