...
public class Commandwarp extends EssentialsCommand
{
	...
	public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		if (args.length == 0 || args[0].matches("[0-9]+"))
		{
			...
			{
				throw new Exception(tl_("warpListPermission"));
			}
	...
	private void warpList(final CommandSource sender, final String[] args, final IUser user) throws Exception
	{
		...
		{
			throw new Exception(tl_("noWarpsDefined"));
		}
		int page = 1;
		...
		{
			sender.sendMessage(tl_("warpsCount", warpNameList.size(), page, maxPages));
			sender.sendMessage(tl_("warpList", warpList));
		}
		else
		{
			sender.sendMessage(tl_("warps", warpList));
		}
	}
	...
	private void warpUser(final User owner, final User user, final String name) throws Exception
	{
		...
		{
			throw new Exception(tl_("warpUsePermission"));
		}
