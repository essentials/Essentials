...
public class Commandunlimited extends EssentialsCommand
{
	...
	private String getList(final User target)
	{
		...
		output.append(tl_("unlimitedItems")).append(" ");
		boolean first = true;
		final List<Integer> items = target.getUnlimited();
		if (items.isEmpty())
		{
			output.append(tl_("none"));
		}
	...
	private Boolean toggleUnlimited(final User user, final User target, final String item) throws Exception
	{
		...
		{
			throw new Exception(tl_("unlimitedItemPermission", itemname));
		}
		...
		if (user != target)
		{
			user.sendMessage(tl_(message, itemname, target.getDisplayName()));
		}
		target.sendMessage(tl_(message, itemname, target.getDisplayName()));
