...
public class Commandsell extends EssentialsCommand
{
	...
	public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		if (count != 1)
		{
			...
			{
				user.sendMessage(tl_("totalWorthBlocks", type, NumberUtil.displayCurrency(totalWorth, ess)));
			}
			else
			{
				user.sendMessage(tl_("totalWorthAll", type, NumberUtil.displayCurrency(totalWorth, ess)));
			}
		}
	}
	...
	private BigDecimal sellItem(User user, ItemStack is, String[] args, boolean isBulkSell) throws Exception
	{
		...
		{
			if (!isBulkSell) {
				user.sendMessage(tl_("itemSold", NumberUtil.displayCurrency(BigDecimal.ZERO, ess), BigDecimal.ZERO, is.getType().toString().toLowerCase(Locale.ENGLISH), NumberUtil.displayCurrency(worth, ess)));
			}
		...
		user.sendMessage(tl_("itemSold", NumberUtil.displayCurrency(result, ess), amount, is.getType().toString().toLowerCase(Locale.ENGLISH), NumberUtil.displayCurrency(worth, ess)));
		logger.log(Level.INFO, tl_("itemSoldConsole", user.getDisplayName(), is.getType().toString().toLowerCase(Locale.ENGLISH), NumberUtil.displayCurrency(result, ess), amount, NumberUtil.displayCurrency(worth, ess)));
		return result;
	}
}
