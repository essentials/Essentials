...
public class Commandclearinventory extends EssentialsCommand
{
	...
	private void parseCommand(Server server, CommandSource sender, String[] args, boolean allowOthers, boolean allowAll) throws Exception
	{
		...
		{
			sender.sendMessage(tl_("inventoryClearingFromAll"));
			offset = 1;
	...
	protected void clearHandler(CommandSource sender, Player player, String[] args, int offset, boolean showExtended) throws Exception
	{
		...
		if (type == -1) // type -1 represents wildcard or all items
		{
			if (showExtended)
			{
				sender.sendMessage(tl_("inventoryClearingAllItems", player.getDisplayName()));
			}
		...
		else if (type == -2) // type -2 represents double wildcard or all items and armor
		{
			if (showExtended)
			{
				sender.sendMessage(tl_("inventoryClearingAllArmor", player.getDisplayName()));
			}
			player.getInventory().clear();
		...
		else
		{
			if (data == -1) // data -1 means that all subtypes will be cleared
			{
				...
				if (showExtended)
				{
					sender.sendMessage(tl_("inventoryClearingAllStack", stack.getType().toString().toLowerCase(Locale.ENGLISH), player.getDisplayName()));
				}
			...
			else if (amount == -1) // amount -1 means all items will be cleared
			{
				...
				{
					sender.sendMessage(tl_("inventoryClearingStack", removedAmount, stack.getType().toString().toLowerCase(Locale.ENGLISH), player.getDisplayName()));
				}
			}
			else
			{
				...
				{
					sender.sendMessage(tl_("inventoryClearingStack", amount, stack.getType().toString().toLowerCase(Locale.ENGLISH), player.getDisplayName()));
				...
				else
				{
					if (showExtended)
					{
						sender.sendMessage(tl_("inventoryClearFail", player.getDisplayName(), amount, stack.getType().toString().toLowerCase(Locale.ENGLISH)));
					}
				}
			}
		}
