...
public class Commandtp extends EssentialsCommand
{
	...
	public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		case 4:
			...
			{
				throw new Exception(tl_("noPerm", "essentials.tp.others"));
			}
			...
			{
				throw new Exception(tl_("teleportDisabled", target2.getDisplayName()));
			}
			...
			target2.sendMessage(tl_("teleporting", loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
			break;
		case 2:
		default:
			if (!user.isAuthorized("essentials.tp.others"))
			...
			{
				throw new Exception(tl_("teleportDisabled", target.getDisplayName()));
			}
			if (!toPlayer.isTeleportEnabled())
			{
				throw new Exception(tl_("teleportDisabled", toPlayer.getDisplayName()));
			}
			...
			{
				throw new Exception(tl_("noPerm", "essentials.worlds." + toPlayer.getWorld().getName()));
			}
			target.getTeleport().now(toPlayer.getBase(), false, TeleportCause.COMMAND);
			target.sendMessage(tl_("teleportAtoB", user.getDisplayName(), toPlayer.getDisplayName()));
			break;
		}
	}
	...
	public void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		...
		if (args.length == 2)
		{
			...
			target.getTeleport().now(toPlayer.getBase(), false, TeleportCause.COMMAND);
			target.sendMessage(tl_("teleportAtoB", Console.NAME, toPlayer.getDisplayName()));
		}
		else if (args.length > 3)
		{
			...
			target.sendMessage(tl_("teleporting", loc.getWorld().getName(), loc.getBlockX(), loc.getBlockY(), loc.getBlockZ()));
