...
public class Commandnear extends EssentialsCommand
{
	...
	protected void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		{
			user.sendMessage(tl_("radiusTooBig", maxRadius));
			radius = maxRadius;
		}
		...
		user.sendMessage(tl_("nearbyPlayers", getLocal(server, otherUser, radius)));
	}
	...
	protected void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		...
		sender.sendMessage(tl_("nearbyPlayers", getLocal(server, otherUser, radius)));
	}
	...
	private String getLocal(final Server server, final User user, final long radius)
	{
		...
		return output.length() > 1 ? output.toString() : tl_("none");
	}
}
