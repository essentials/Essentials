...
public class Commandweather extends EssentialsCommand
{
	...
	public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		if (args.length > 1)
		{
			...
			user.sendMessage(isStorm
							 ? tl_("weatherStormFor", world.getName(), args[1])
							 : tl_("weatherSunFor", world.getName(), args[1]));
		}
		else
		{
			world.setStorm(isStorm);
			user.sendMessage(isStorm
							 ? tl_("weatherStorm", world.getName())
							 : tl_("weatherSun", world.getName()));
		}
	}
	...
	protected void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		...
		if (world == null)
		{
			throw new Exception(tl_("weatherInvalidWorldWorld", args[0]));
		}
		if (args.length > 2)
		{
			...
			sender.sendMessage(isStorm
							   ? tl_("weatherStormFor", world.getName(), args[2])
							   : tl_("weatherSunFor", world.getName(), args[2]));
		}
		else
		{
			world.setStorm(isStorm);
