...
public class Commandkit extends EssentialsCommand
{
	...
	public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		if (args.length < 1)
		{
			...
			user.sendMessage(kitList.length() > 0 ? tl_("kits", kitList) : tl_("noKits"));
	...
	public void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		if (args.length < 2)
		{
			...
			sender.sendMessage(kitList.length() > 0 ? tl_("kits", kitList) : tl_("noKits"));
			throw new NoChargeException();
		}
		else
		{
			...
			sender.sendMessage(tl_("kitGiveTo", kitName, userTo.getDisplayName()));
			userTo.sendMessage(tl_("kitReceive", kitName));
		}
	}
	...
	{		
		if (kitName.isEmpty())
		{
			throw new Exception(tl_("kitError2"));
		}
		...
		{
			throw new Exception(tl_("noKitPermission", "essentials.kits." + kitName));
		}
		...
		userFrom.sendMessage(tl_("kitGiveTo", kitName, userTo.getDisplayName()));
		userTo.sendMessage(tl_("kitReceive", kitName));
	}
}
