...
public class Commandrecipe extends EssentialsCommand
{
	...
	public void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		...
		if (args.length > 1)
		{
			...
			{
				throw new Exception(tl_("invalidNumber"));
			}
		}
		...
		{
			throw new Exception(tl_("recipeNone", getMaterialName(itemType)));
		}

		if (recipeNo < 0 || recipeNo >= recipesOfType.size())
		{
			throw new Exception(tl_("recipeBadIndex"));
		}

		final Recipe selectedRecipe = recipesOfType.get(recipeNo);
		sender.sendMessage(tl_("recipe", getMaterialName(itemType), recipeNo + 1, recipesOfType.size()));
		...
		{
			sender.sendMessage(tl_("recipeMore", commandLabel, args[0], getMaterialName(itemType)));
		}
	}

	public void furnaceRecipe(final CommandSource sender, final FurnaceRecipe recipe)
	{
		sender.sendMessage(tl_("recipeFurnace", getMaterialName(recipe.getInput())));
	}
	...
	public void shapedRecipe(final CommandSource sender, final ShapedRecipe recipe)
	{
		...
		else
		{
			...
			sender.sendMessage(tl_("recipeGrid", colorMap.get(materials[0][0]), colorMap.get(materials[0][1]), colorMap.get(materials[0][2])));
			sender.sendMessage(tl_("recipeGrid", colorMap.get(materials[1][0]), colorMap.get(materials[1][1]), colorMap.get(materials[1][2])));
			sender.sendMessage(tl_("recipeGrid", colorMap.get(materials[2][0]), colorMap.get(materials[2][1]), colorMap.get(materials[2][2])));

			StringBuilder s = new StringBuilder();
			for (Material items : colorMap.keySet().toArray(new Material[colorMap.size()]))
			{
				s.append(tl_("recipeGridItem", colorMap.get(items), getMaterialName(items)));
			}
			sender.sendMessage(tl_("recipeWhere", s.toString()));
		}
	}
	...
	public void shapelessRecipe(final CommandSource sender, final ShapelessRecipe recipe)
	{
		...
		else
		{
			...
			sender.sendMessage(tl_("recipeShapeless", s.toString()));
		}
	}
