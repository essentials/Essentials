...
public class Commandhome extends EssentialsCommand
{
	...
	public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		try
		{
			if ("bed".equalsIgnoreCase(homeName) && user.isAuthorized("essentials.home.bed"))
			{
				...
				{
					throw new Exception(tl_("bedMissing"));
				}
			}
		...
		catch (NotEnoughArgumentsException e)
		{
			...
			{
				throw new Exception(tl_("noHomeSetPlayer"));
			}
			...
			else
			{
				...
				if (user.isAuthorized("essentials.home.bed"))
				{
					...
					{
						homes.add(tl_("bedNull"));
					}
				}
				user.sendMessage(tl_("homes", StringUtil.joinList(homes), count, getHomeLimit(player)));
			}
		}
		throw new NoChargeException();
	}
	...
	{		
		...
		{
			throw new Exception(tl_("noPerm", "essentials.worlds." + loc.getWorld().getName()));
		}
