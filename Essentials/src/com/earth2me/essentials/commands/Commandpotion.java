...
public class Commandpotion extends EssentialsCommand
{
	...
	protected void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		if (args.length == 0)
		{
			...
			throw new NotEnoughArgumentsException(tl_("potions", StringUtil.joinList(potionslist.toArray())));
		}
		...
		if (stack.getType() == Material.POTION)
		{
			...
			if (args.length > 0)
			{
				...
				else
				{
					...
					{
						user.sendMessage(tl_("invalidPotion"));
						throw new NotEnoughArgumentsException();
					}
				}
			}
		...
		{
			throw new Exception(tl_("holdPotion"));
		}
	}
}
