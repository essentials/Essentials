...
public class Commandsocialspy extends EssentialsToggleCommand
{
	...
	void togglePlayer(CommandSource sender, User user, Boolean enabled) throws NotEnoughArgumentsException
	{
		...
		user.sendMessage(tl_("socialSpy", user.getDisplayName(), enabled ? tl_("enabled") : tl_("disabled")));
		if (!sender.isPlayer() || !sender.getPlayer().equals(user.getBase()))
		{
			sender.sendMessage(tl_("socialSpy", user.getDisplayName(), enabled ? tl_("enabled") : tl_("disabled")));
		}
	}
}
