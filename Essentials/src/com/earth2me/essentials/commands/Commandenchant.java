...
public class Commandenchant extends EssentialsCommand
{
	...
	protected void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		{
			throw new Exception(tl_("nothingInHand"));
		}
		if (args.length == 0)
		{
			...
			throw new NotEnoughArgumentsException(tl_("enchantments", StringUtil.joinList(enchantmentslist.toArray())));
		}
		...
		if (level == 0)
		{
			user.sendMessage(tl_("enchantmentRemoved", enchantmentName.replace('_', ' ')));
		}
		else
		{
			user.sendMessage(tl_("enchantmentApplied", enchantmentName.replace('_', ' ')));
		}
	}
}
