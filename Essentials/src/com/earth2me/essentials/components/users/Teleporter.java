...
public class Teleporter implements Runnable, ITeleporter
{
	...
	public Teleporter(IUserComponent user, IContext ess)
	{
		this.user = user;
		this.context = ess;
	}
	...
	private void teleport(Target target, Trade chargeFor, TeleportCause cause) throws Exception
	{
		...
		cooldown(true);
		if (teleportDelay <= 0 || Permissions.TELEPORT_TIMER_BYPASS.isAuthorized(user))
		{
			cooldown(false);
			now(target, cause);
			if (chargeFor != null)
