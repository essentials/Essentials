...
public class HelpInput implements IText
{
	...
	public HelpInput(final User user, final String match, final IEssentials ess) throws IOException
	{
		...
		{
			lines.add(tl_("helpMatching", match));
		}
		...
		for (Plugin p : ess.getServer().getPluginManager().getPlugins())
		{
			try
			{
				...
				if (pluginNameLow.equals(match))
				{
					lines.clear();
					newLines.clear();
					lines.add(tl_("helpFrom", p.getDescription().getName()));
				}
				...
				for (Map.Entry<String, Map<String, Object>> k : cmds.entrySet())
				{
					try
					{
						...
						if (pluginNameLow.contains("essentials"))
						{
							...
							{
								pluginLines.add(tl_("helpLine", k.getKey(), k.getValue().get(DESCRIPTION)));
							}
						}
						else
						{
							if (ess.getSettings().showNonEssCommandsInHelp())
							{
								...
								{
									pluginLines.add(tl_("helpLine", k.getKey(), value.get(DESCRIPTION)));
								}
				...
				if (!pluginLines.isEmpty())
				{
					...
					if (match.equalsIgnoreCase(""))
					{
						lines.add(tl_("helpPlugin", pluginName, pluginNameLow));
					}
				}
			}
			catch (NullPointerException ex)
			...
			catch (Exception ex)
			{
				if (!reported)
				{
					logger.log(Level.WARNING, tl_("commandHelpFailedForPlugin", pluginNameLow), ex);
				}
				reported = true;
			}
		}
