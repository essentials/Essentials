	...
	public void showPage(final String pageStr, final String chapterPageStr, final String commandName, final CommandSource sender)
	{
		...
		if (pageStr == null || pageStr.isEmpty() || pageStr.matches("[0-9]+"))
		{
			...
			if (!lines.isEmpty() && lines.get(0).startsWith("#"))
			{
				...
				sender.sendMessage(tl_("infoChapter"));
			...
			else
			{
				...
				if (!onePage && commandName != null)
				{
					...
					sender.sendMessage(tl_("infoPages", page, pages, content));
				}
				...
				{
					sender.sendMessage(tl_("readNextPage", commandName, page + 1));
				}
				return;
			}
		}
		...
		{
			sender.sendMessage(tl_("infoUnknownChapter"));
			return;
		}
		...
		if (!onePage && commandName != null)
		{
			...
			sender.sendMessage(tl_("infoChapterPages", content, page, pages));
		}
		...
		if (!onePage && page < pages && commandName != null)
		{
			sender.sendMessage(tl_("readNextPage", commandName, pageStr + " " + (page + 1)));
		}
	}
}
