...
public class Warps implements IConf, net.ess3.api.IWarps
{
	...
	public void setWarp(String name, Location loc) throws Exception
	{
		...
		if (conf == null)
		{
			...
			if (confFile.exists())
			{
				throw new Exception(tl_("similarWarpExist"));
			}
		...
		{
			throw new IOException(tl_("invalidWarpName"));
		}
	}
	...
	public void removeWarp(String name) throws Exception
	{
		...
		if (conf == null)
		{
			throw new Exception(tl_("warpNotExist"));
		}
		if (!conf.getFile().delete())
		{
			throw new Exception(tl_("warpDeleteError"));
		}
		...
		if (listOfFiles.length >= 1)
		{
			for (int i = 0; i < listOfFiles.length; i++)
			{
				...
				if (listOfFiles[i].isFile() && filename.endsWith(".yml"))
				{
					...
					{
						logger.log(Level.WARNING, tl_("loadWarpError", filename), ex);
					}
				}
			}
		}
