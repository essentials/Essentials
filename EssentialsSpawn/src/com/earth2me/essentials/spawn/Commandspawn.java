...
public class Commandspawn extends EssentialsCommand
{
	...
	public void run(final Server server, final User user, final String commandLabel, final String[] args) throws Exception
	{
		...
		if (args.length > 0 && user.isAuthorized("essentials.spawn.others"))
		{
			...
			{
				otherUser.sendMessage(tl_("teleportAtoB", user.getDisplayName(), "spawn"));
			}
		}
		else
		{
	...
	protected void run(final Server server, final CommandSource sender, final String commandLabel, final String[] args) throws Exception
	{
		...
		user.sendMessage(tl_("teleportAtoB", Console.NAME, "spawn"));
	...
	private void respawn(final CommandSource sender, final User teleportOwner, final User teleportee, final Trade charge) throws Exception
	{
		...
		sender.sendMessage(tl_("teleporting", spawn.getWorld().getName(), spawn.getBlockX(), spawn.getBlockY(), spawn.getBlockZ()));
